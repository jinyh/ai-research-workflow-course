...
| [slides.md](./slides.md) | 现行·逐页母稿 v0.2.0（17 页，草稿） | 逐页屏显文案、视觉结构、讲述备注、互动、时间、来源与事实边界 |
...
| 2. 内容门 | ✅ 通过（三件套已对齐口径；讲义为内容源；关键结论绑定可核验来源——Booth et al. 2024 / Whitesides 2004 / Gopen & Swan 1990 / Peyton Jones 讲义 / AGENTS.md / assignments.md Checkpoint 4；AI 输出仅作线索；失败不选择性删除对齐 AGENTS.md 与 assessment.md 学术规范红线；v0.2.0 已做保守审校——合并页标题降格为指引式、备注未引入 meta 自白；不动 handout 正文/案例/证据；保留 AI 使用披露七字段与失败不选择性删除口径；待教师终审） |
| 3. 90 分钟教学门 | ✅ 通过（已对齐备课规划第 14 课权威 5 段时间表 0-15/15-35/35-60/60-78/78-90；页量预算 17 页，段内均分；原 18 页合并为 17 页——旧 P12+P13 合并为署名/引用/AI 输出线索页；第一次学生动手 35 分钟，最小产出=论文式短文初版 IMRaD 骨架+贡献列表+结论追踪表+AI 使用披露段；备用路径已标；桌面推演待续） |
| 4. 逐页映射门 | ✅ 通过（17 页映射表已复核，每页标 handout 小节；90 分钟节奏表页码列与正文页码引用已同步；P12 合并 AI 输出+署名+引用证据角色，来源去重；待教师终审） |
...
- `备课规划.md` 第 14 课段已补逐时间表（v0.2.0 slides 已按权威 5 段 0-15/15-35/35-60/60-78/78-90 对齐，段内均分；本项已解决）；
...
- 页数密度：v0.2.0 已从 18 页合并为 17 页（旧 P12+P13 合并），对照权威 5 段预算；桌面推演后是否再调整待教师终审；
