...
第 3 课把检索过程变成可复盘的方法，并为每条材料标明证据角色。但只标角色还不够。一条被标为"主证据"的论文，如果你只读了 AI 摘要而没有回到原文核验任务、设置、指标、对照和限定条件，它仍然可能成为错读的起点。
...
八阶段研究链路中，本课承接阶段四"外部输入摄取"，并向阶段五"证据整理"输出经过核验的材料。第 3 课解决了"材料从哪来、能支撑什么"；本课解决"这篇论文到底说了什么、说到什么程度"。

> 与第 3 课的连接：第 3 课的候选文献表里，`verified` 条目只表示题名、作者、DOI 与原文一致，不表示你已经理解了它的贡献和局限。本课把 `verified` 的论文进一步精读，产出阅读卡，为第 5 课的证据地图提供可追溯的主张—位置对。
...
记录至少一处 AI 的错误、过度概括、遗漏或高风险判断。如果未发现明显错误，记录检查过的风险点及核验依据，不虚构错误。
...
> 与第 3 课的连接：第 3 课的来源审计处理"AI 给出的文献是否真实存在"；本课处理"AI 复述的内容是否与原文一致"。前者是入口核验，后者是内容核验。
...
- **AI 偏差审计**：至少记录一处 AI 错读、过度概括、位置错指或伪造引用；未发现明显错误时记录检查过的风险点。
- **核验状态**：`pending`（尚未核验）/ `verified`（主张与原文一致）/ `verified-with-caveat`（一致但需限定）/ `rejected`（与原文不符）。

阅读卡保存在个人项目 `notes/reading-cards/` 或 `evidence-map.md` 的论文区段。第 6 课问题门统一检查至少 3 张与个人项目直接相关的完整精读卡。
...
仍以第 1-3 课贯穿案例为例。假设候选文献表里有一条 `verified` 论文，标题与 DOI 都已核验。研究者要为它建立阅读卡。

### 第一步：AI 导航

让 AI 给出中心问题、论证结构、关键术语、预期证据位置和待追问问题。AI 输出："中心问题是结构化阅读卡是否降低 AI 摘要的限定条件遗漏率；见图 2 的对照实验"。全部标记"未核验"。

### 第二步：原文核验

人工阅读摘要、§3 方法、§4 实验和 §5 讨论。核验发现：

- 中心问题与摘要一致，但原文强调"在计算机领域论文上"——AI 导航遗漏了这个限定；
- 图 2 讨论的是"整体遗漏率"，不是"对照实验"——AI 指向错了，对照实验在 §4.2, Table 1；
- 原文措辞是"降低"，不是"消除"；
- 样本规模为 50 篇论文，作者在 §5.2 自述"外部有效性有限"。

记录原文位置：§3 p.4 方法表、§4.2 p.6 Table 1 对照、§5.2 p.9 局限。

### 第三步：AI 质疑

让 AI 根据已核验内容提出反例和遗漏。AI 提出："50 篇样本是否足以支撑结论强度？评价者一致性如何？"回到原文 §4.3 找到评价者一致性 kappa 值，记录。
...
| AI 输出 | 偏差类型 | 原文实际内容 | 核验位置 | 处理 |
| --- | --- | --- | --- | --- |
| "见图 2 的对照实验" | 位置错指 | 图 2 是整体遗漏率，对照实验在 Table 1 | p.6, Fig.2; p.6, Table 1 | 重新定位 |
| "降低 AI 摘要的限定条件遗漏率"（无限定） | 限定遗漏 | 原文限定为"计算机领域论文" | 摘要, §5.2 | 补限定 |

### 第五步：人工定稿

写出阅读卡：作者主张（带位置）、原文证据（指标、对照、样本、限定）、局限与边界（作者自述 + 个人判断）、个人判断（是否能用于当前项目）、AI 偏差审计、核验状态 `verified-with-caveat`。

本课不要求填满所有字段，只要求形成一张可继续扩展的阅读卡。
...
对 AI 导航输出逐条核验，填写最小偏差审计表。至少记录一处 AI 错读、位置错指或过度概括。未发现明显错误时，记录检查过的风险点及核验依据。
...
在项目 `notes/reading-cards/` 或 `evidence-map.md` 论文区段写入一张完整阅读卡。至少包含：作者主张（带位置）、原文证据、局限与边界、个人判断、AI 偏差审计、核验状态。本课要求 2-3 张，只要求形成可继续扩展的起点。
