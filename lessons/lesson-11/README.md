...
| 2. 内容门 | ✅ 通过（三件套已对齐口径；讲义为内容源；关键结论绑定可核验来源——Sandve et al. 2013 / Lu et al. Nature 2026 / Huang et al. MLAgentBench ICML 2024 / Karpathy AutoResearch；AutoResearch / AI Scientist 只作分析对象，不外推为通用自主科研；AI 输出仅作线索；slides gate2 保守审校已完成——去 meta 自白 3 处、标题腔降格 1 处、对仗句式去密 1 处，保留 3-4 处张力；待跨文档检查与教师终审） |
| 3. 90 分钟教学门 | ✅ 通过（`备课规划.md` 第 11 课权威六段逐时间表已补——0-18/18-35/35-55/55-75/75-85/85-90；slides 90 分钟节奏表已对齐六段；每页建议时间按段内均分重排（首页 0:00、末页 90:00）；段4 停留页合并后 2 页各 10 分钟；第一次学生动手 55 分钟，最小产出=受限循环说明（四约束+五步骨架+审核点）+ 失败迭代记录（七字段）+ 八维拆解表（≥4 维）；备用路径已标；桌面推演待续） |
| 4. 逐页映射门 | ✅ 通过（19 页映射表已同步——段4 停留页合并（旧 P17+P18→新 P17），旧 P19→P18、旧 P20→P19 重编号；每页标 handout 小节；待教师复核） |
...
- 19 页是否在桌面推演后调整页数密度（第 9-10 课同为 20 页）；
