...
| [slides.md](./slides.md) | 现行·逐页母稿 v0.2.0（14 页，草稿） | 逐页屏显文案、视觉结构、讲述备注、互动、时间、来源与事实边界 |
...
| 2. 内容门 | ✅ 保守审校通过（三件套对齐口径；讲义为内容源；关键结论绑定可核验来源——Booth et al. / Whitesides / Peyton Jones / assignments.md 判断门条件；AI 输出仅作线索；已去 meta 自白与对仗去密，保留 3-4 处张力；形成性不提交口径不动） |
| 3. 90 分钟教学门 | ✅ 通过（节奏表对齐 `备课规划.md` v2.1.0 第 8 课段逐时间表权威 8 段；方法段 10 页合并为 6 页，全文 18→14 页；每页建议时间按段时长均分；第一次学生动手 40 分钟；最小产出=分享页+反馈记录表+采纳决策表 2 条（含 1 采纳 1 拒绝/暂缓）+判断门预检缺口清单+一条 AI 使用记录；备用路径已标） |
| 4. 逐页映射门 | ✅ 通过（14 页映射表，每页标 handout 小节；方法段合并后映射表、90 分钟节奏表页码列与正文页码引用已同步；P10 演示展开 P04 压缩的贯穿案例五段） |
...
- 与第 9 课判断门的衔接点：判断门材料包（baseline、实验规格完整字段、复现说明、迁移到 project-template.md）的迁移路径是否在 slides P14 进一步细化；
- 与第 7 课的衔接点：第 7 课产出（机制假设列表、研究判断初版、实验规格草图）到第 8 课 3 分钟陈述五段的对应是否在 slides P10 进一步细化；
