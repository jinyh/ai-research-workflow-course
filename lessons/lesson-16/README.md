...
| 2. 内容门 | 🟡 草稿通过（三件套已对齐口径；讲义为内容源；关键结论绑定可核验来源——Alley 2013 / Peyton Jones 讲义 / Tao ICM 2026 / Booth et al. / Smith 1990 / assignments.md 门条件 / AGENTS.md 证据标准；AI 输出仅作线索；学术规范红线对齐 assessment.md；不残留旧口径——证据三角硬门槛/每周作业/默认小组提交均已清除；待第二轮审校去 AI 痕迹与跨文档检查） |
| 3. 90 分钟教学门 | 🟡 草稿（备课规划 第 16 课段未给逐时间表，教案按默认节奏构造，与第 6、9、13 课四门风格一致，并兼顾学生陈述与复现抽查；第一次学生动手 25 分钟，最小产出=report.md 收口版 + 最终工作流收口确认 + 工件状态一致性 + 复盘初版；备用路径已标；陈述超时用分会场/海报/短视频；桌面推演待续） |
...
- **论证门+期末项目条件对齐**：handout §八·2 的 10 项门条件与 [assignments.md](../../course/assignments.md) Checkpoint 4 逐项对齐，已核对一致：
  ...
  提交方式（链接/tag/压缩包）、教师抽查（一次复现/一个引用/一条 trace）、未通过处理（局部问题可修订，学术规范红线不适用修订机制）和评分维度（"表达、伦理与复盘"20%；四维度合计 100%）均对齐。**待教师在正式提交前复核 10 项与 assignments.md Checkpoint 4 的逐项一致性（评分依据，不能自造）。**

- **评分合计 100% 对齐**：四维度已核对——文献与问题定位 25% + 实验设计与可复现性 30% + 原型与 Agent Workflow 实践 25% + 表达、伦理与复盘 20% = 100%。本课对应"表达、伦理与复盘"20%，同时前三门（问题门/判断门/验证门）的过程证据在此汇总。**待教师复核评分比例与 assessment.md "总体结构"表一致。**

- `备课规划.md` 第 16 课段是否补逐时间表（当前教案按默认节奏构造，与第 6、9、13 课风格一致，但非权威源给定）；
...
- 20 页是否在桌面推演后调整页数密度（第 6、9、13 课同为 20 页）；
