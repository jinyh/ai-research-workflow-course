...
在项目 `evidence-map.md` 中完成：1 个子问题；2 张 `verified` 或带完整限定的 `verified-with-caveat` 阅读卡；1 个窄矩阵；2 条证据地图主格；4 句 mini literature review；1 条 AI 使用记录。如果发现空白候选，当堂只记它的类型、当前状态（通常为 `pending`）和下一步窄检索；真实性、重要性与可行性三步完整检验在第 6 课前补齐。
