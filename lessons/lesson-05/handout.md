...
完成本课后，你将形成一份 mini literature review 最小可行草稿和证据地图初版，并带着研究空白候选进入第 6 课，把它们作为问题门的直接输入。
...
四句话必须能回指证据地图中的已核验条目；pending、rejected 和来源不可追溯的内容只能留在探索线索区。本课不要求形成完整综述，也不要求润色为论文 related work 段。
...
本课不要求填满证据地图，也不要求写成完整 related work；只要求跑通一次"阅读卡 → 矩阵 → 地图 → 空白候选 → 三步检验 → 四句 mini review"的可复盘链路。
...
在项目 `evidence-map.md` 中写入：相关工作矩阵 + 证据地图初版 + 研究空白候选清单 + 四句 mini literature review 最小可行草稿。本课不要求填满，只要求形成可继续扩展的起点。第 6 课问题门统一检查。
