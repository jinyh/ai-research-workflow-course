...
完成本课后，你将带着一份证据地图初版和研究空白候选进入第 6 课，把它们作为问题门的直接输入。
...
3. 绘制证据地图，显式区分主证据、补充证据、对比证据和证据空白，并保留冲突；
4. 识别冲突空白、缺角空白和新问题空白三类研究空白，并判断哪些是真实问题、哪些只是表述差异；
5. 对 AI 辅助归纳出的综述草稿执行核验，把虚构引文、过度概括和来源不可追溯的条目归位为探索线索；
6. 说明证据地图与研究空白为何是问题门的直接输入。
...
| --- | --- | --- |
| 候选文献表（含证据角色和核验状态） | 按研究问题重组为相关工作矩阵 | 证据地图初版 |
| 核心论文阅读卡（含原文位置） | 抽取主张、证据、设置和局限进入证据地图 | 研究空白候选 |
| AI 摘要错读审计记录 | 对 AI 综述草稿执行相同核验清单 | 一条 AI 使用记录 |

> 与第 1 课的连接：第 1 课 §2.5 给出了一个简化证据地图样例（E1/E2/E3），本课把它扩展为完整的四类标注，并显式保留冲突和缺失。

证据整理不等于文献综述的最终写作。本课产出的是供研究者自己判断和供问题门检查的内部工件，不是论文 related work 段的成稿。写作层面的论证结构在第 14 课展开。
...
第 3 课给了证据角色的四类（主证据、补充证据、对比证据、探索线索）。证据地图在此基础上增加第四类"证据空白"，并要求每条材料显式标位置、角色和冲突关系。

| 类别 | 定义 | 在地图中能做什么 | 不能做什么 |
| --- | --- | --- | --- |
| 主证据 | 直接支持或反驳当前机制假设、来自可核验一手来源 | 支撑核心主张 | 不能单独支撑外推 |
| 补充证据 | 从不同来源、测量或路径补充支持同一主张 | 提供独立性 | 不能替代主证据回答核心因果问题 |
| 对比证据 | 提供替代解释、竞争假设或反例 | 暴露当前解释的边界 | 不能因为相反就自动推翻主证据 |
| 证据空白 | 当前地图中没有可直接回答某子问题的核验材料 | 指向下一步检索或研究空白候选 | 不能直接写成"已有方法无效" |

几个要点：

1. 冲突必须显式呈现。如果论文 A 支持、论文 B 反对同一主张，地图里两格都要保留，并记录取舍理由或"待核验"。
2. "独立"不等于两个不同网页（见第 3 课）。两篇互相转载、同一数据集、同一团队的不同论文都不自动构成独立证据。
3. 证据空白不是"领域空白"的同义词。它只指在你当前地图范围内、针对你当前子问题，没有可直接对应的核验材料。领域是否真的没人做过，要进一步检索才能判断。
...
- 核验状态沿用第 3 课的 pending / verified / verified-with-caveat / rejected；只有 verified 条目才进入主格。
...
通过三步检验的候选，进入问题门材料的"研究空白候选"清单，由第 6 课第一性原理分析进一步推导为可证伪命题。

> 来源参考：Qian, Z. *How to Look for Ideas in Computer Science Research*. 本课只分析其维度表和 gap 候选生成逻辑；"表格空位"不能自动证明问题重要或真实（见 [reading-list.md](../../course/reading-list.md) 第 5 课课堂案例）。
...
8. **证据地图当成 related work 初稿**：地图保留冲突、空白和未核验条目，成稿综述要面向读者取舍，二者不同。

---

## 七、贯穿案例：从阅读卡到证据地图

仍以第 1 课案例为例。假设第 4 课已为 5 篇文献建立了阅读卡。本课的动作链：
...
第三步，从矩阵抽取条目进入证据地图：A 为主证据（直接回答 a、b），B 为补充证据（部分回答 a，不回答 b），C 为对比证据（回答 b 但任务不同）。D 列整列空白——这是缺角空白候选。

第四步，对 D 执行窄检索：构造检索式 `"reading card" AND "omission rate" AND "computer science"`，记录库、日期、命中数。若命中数为 0 或全部不可比，进入下一步；否则补入地图。
...
用你第 4 课的 2-3 张阅读卡，写出你的研究问题的 3 个子问题，填入矩阵。至少一格标"证据空白候选"并说明是"没找到"还是"不可比"还是"确实没人做过"。

### 练习 3：标注证据地图

从练习 2 的矩阵中抽取条目，填入证据地图最小字段。至少包含一条主证据、一条对比证据和一条冲突（如确无冲突，说明为什么）。

### 练习 4：识别空白类型

为你找到的一个证据空白候选标注类型（冲突空白/缺角空白/新问题空白），并执行 §四·2 的三步检验。哪一步它最可能不通过？
...
在项目 `evidence-map.md` 中写入：相关工作矩阵 + 证据地图初版 + 研究空白候选清单。本课不要求填满，只要求形成可继续扩展的起点。第 6 课问题门统一检查。
...
| 证据地图 | 显式区分主证据、补充证据、对比证据和证据空白的材料组织，保留冲突和未核验条目 |
...
4. Qian, Z. *How to Look for Ideas in Computer Science Research*. 本课只分析维度表和 gap 候选生成逻辑；"表格空位"不能自动证明问题重要或真实。
